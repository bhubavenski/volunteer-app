...
const StablePDFDownload = memo(({ formData, selectedPdf, isFormValid }: StablePDFDownloadProps) => {
...
const PDFCard = memo(({ pdf, isSelected, isDeleting, onLoad, onDelete }: PDFCardProps) => {
  // console.log(`Rendering card for ${pdf.title}`);
  
  return (
    <Card className={cn("cursor-pointer hover:bg-muted/50", isSelected ? "border-primary" : "")}>
      <CardContent className="p-4">
        <div className="flex justify-between items-start">
          <div className="flex-1" onClick={() => onLoad(pdf)}>
            <h3 className="font-medium truncate">{pdf.title}</h3>
            <p className="text-sm text-muted-foreground">{format(new Date(pdf.createdAt), "dd.MM.yyyy HH:mm")}</p>
...
const HistoryList = memo(
  ({ history, searchTerm, isLoading, isDeleting, onLoadPdf, onDeletePdf, selectedPdfId }: HistoryListProps) => {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)}>
                        ...
                        <PopoverTrigger asChild>
                          <FormControl>
                            ...
                            >
                              ...
                              {field.value ? format(field.value, "PPP", { locale: bg }) : "Изберете дата"}
                            </Button>
                          </FormControl>
                        </PopoverTrigger>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea placeholder="Въведете описание" rows={4} {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Въведете име на организатора" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Въведете локация" {...field} value={field.value || ""} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Въведете контактна информация" {...field} value={field.value || ""} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <CardFooter className="flex justify-between max-sm:flex-col">
                ...
                <div className="flex space-x-2">
                  ...
                  <StablePDFDownload formData={formValues} selectedPdf={selectedPdf} isFormValid={isFormValid} />
                </div>
              </CardFooter>
            </form>
          </Form>
