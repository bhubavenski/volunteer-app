...
export const CategoriesInput: React.FC<CategoriesInputProps> = ({ onCategoriesChange }) => {
      ...
      {categories.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {categories.map((category, index) => (
            ...
            >
              {category}
              <button type="button" onClick={() => removeCategory(category)} className="ml-2 focus:outline-none">
                <X size={14} />
              </button>
            </div>
          ))}
